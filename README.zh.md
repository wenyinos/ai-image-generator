...
- 百炼 qwen-image-3.0 / qwen-image-3.0-pro、wan2.7-image / wan2.7-image-pro 文生图走异步任务轮询：显示"排队中/生成中 + 已等待时长"
- DashScope 模型快照输入：选择模型 + 输入快照日期（如 `2026-06-22`）自动拼接为 `模型-快照`（如 `qwen-image-2.0-pro-2026-06-22`），不填则保留原模型名

### 图生图
- 支持拖拽/点击上传，前端自动压缩（上传上限 10MB）
- 上传进度实时显示；上传完成后显示"AI 正在生成 + 已等待时长"
- 百炼 qwen-image-3.0 / qwen-image-3.0-pro、wan2.7-image / wan2.7-image-pro 图生图走异步任务轮询：显示"排队中/生成中 + 已等待时长"，不再长时间停留在"上传 100%"
