...
`);
// 旧库迁移：补充 video_id 列（Agnes 任务按规范用 /agnesapi?video_id= 查询）
if (!videoTaskDb.prepare('PRAGMA table_info(video_tasks)').all().some((col) => col.name === 'video_id')) {
  videoTaskDb.exec('ALTER TABLE video_tasks ADD COLUMN video_id TEXT');
}
videoTaskDb.exec(`
...
const videoTaskUpsertStmt = videoTaskDb.prepare(`
  INSERT INTO video_tasks (
    task_id, video_id, provider, model, mode, status, prompt, duration, resolution, ratio,
    video_url, usage_json, error_message, created_at, updated_at
  ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  ON CONFLICT(task_id) DO UPDATE SET
    video_id = COALESCE(NULLIF(excluded.video_id, ''), video_tasks.video_id),
