...
function listHistoryRecords({ type, limit = 100, offset = 0 } = {}) {
  const safeLimit = Math.max(1, Math.min(500, limit));
  const safeOffset = Math.max(0, offset);
  if (type === 'image' || type === 'video') {
    return videoTaskDb
      .prepare('SELECT * FROM history WHERE type = ? ORDER BY created_at DESC LIMIT ? OFFSET ?')
      .all(type, safeLimit, safeOffset)
