...
function fetchThroughProxy(url, options = {}, proxyUrl, timeoutMs) {
  return new Promise((resolve, reject) => {
    ...
    let settled = false;
    // 统一收口：只 settle 一次，清理超时定时器并销毁底层连接
    ...
    const done = (response) => {
      response.on('error', (err) => rejectAndDestroy(err));
      const chunks = [];
    ...
    const sendRequest = (req) => {
      req.on('error', (err) => rejectAndDestroy(err));
    ...
    connectReq.on('connect', (res, connectSocket) => {
      if (res.statusCode !== 200) {
        connectSocket.destroy();
        return rejectAndDestroy(new Error(`代理 CONNECT 失败: ${res.statusCode}`));
      }
      ...
      if (isHttps) {
        ...
        tlsSocket.on('error', (err) => rejectAndDestroy(err));
      } else {
    ...
    connectReq.on('error', (err) => rejectAndDestroy(err));
    connectReq.end();

    if (timeoutMs && timeoutMs > 0) {
      timeoutTimer = setTimeout(() => rejectAndDestroy(new Error('请求超时')), timeoutMs);
    }
  });
}
...
// 安全解析上游 JSON：解析失败时带上 HTTP 状态码抛错，避免 SyntaxError 丢失状态码
async function safeJson(res, fallbackMessage = '上游返回了非 JSON 响应') {
  const text = await res.text();
  try {
    return JSON.parse(text);
  } catch (e) {
    const preview = text.trim().slice(0, 120).replace(/\s+/g, ' ');
    throw new Error(`${fallbackMessage} (HTTP ${res.status})${preview ? `: ${preview}` : ''}`);
  }
}
...
function getDotenvCredential(name) {
  // 优先使用启动时解析好的内存值，避免每个请求同步读整个 .env 文件
  const parsedValue = typeof DOTENV_PARSED[name] === 'string' ? DOTENV_PARSED[name].trim() : '';
  if (parsedValue && !isPlaceholderCredential(parsedValue)) return parsedValue;
  // 兜底：.env 在启动后才被修改（热更新）的场景，重新读一次文件
  let fileValue = '';
  try {
...
// multipart 表单字段值需剥离 CR/LF，防止注入伪造 part 边界头
  ...
  for (let i = 0; i < requestCount; i += 1) {
    ...
    if (parsedImage) {
      ...
      const header = Buffer.from([
        ...
        `--${boundary}\r\n`,
        `Content-Disposition: form-data; name="prompt"\r\n\r\n${sanitizeMultipartFieldValue((prompt && prompt.trim()) ? prompt.trim() : 'Edit the image')}\r\n`,
        `--${boundary}\r\n`,
  ...
  for (let i = 0; i < requestCount; i += 1) {
    ...
    if (imageDataUrl) {
      ...
      if (size) {
        parts.push(
          `--${boundary}\r\n`,
          `Content-Disposition: form-data; name="size"\r\n\r\n${sanitizeMultipartFieldValue(size)}\r\n`
        );
      }
      parts.push(
        `--${boundary}\r\n`,
...
async function fetchAgnesVideoResult({ apiKey, taskId, videoId, modelName }) {
  const baseUrl = process.env.AGNES_BASE_URL || 'https://apihub.agnes-ai.com';
  const timeoutMs = Number.parseInt(process.env.AGNES_TIMEOUT_MS || String(GENERATION_REQUEST_TIMEOUT_MS), 10);
  // 文档推荐查询端点（2.5 系列必须带 video_id + model_name）；无 video_id 时退回旧版兼容端点
  const queryUrl = videoId
    ? `${baseUrl}/agnesapi?video_id=${encodeURIComponent(videoId)}${modelName ? `&model_name=${encodeURIComponent(modelName)}` : ''}`
  ...
  console.log('[agnes] video query response:', JSON.stringify(data).slice(0, 500));
  return data;
}

// 兼容不同代次/站点响应中视频 URL 的字段位置（文档为 metadata.url，旧版为 video_url）
function extractAgnesVideoUrl(data) {
  if (!data || typeof data !== 'object') return '';
  if (typeof data.metadata?.url === 'string' && data.metadata.url) return data.metadata.url;
  if (typeof data.metadata?.video_url === 'string' && data.metadata.video_url) return data.metadata.video_url;
  if (typeof data.video_url === 'string' && data.video_url) return data.video_url;
  if (typeof data.url === 'string' && data.url) return data.url;
  if (Array.isArray(data.data)) {
    const first = data.data.find((item) => typeof item?.url === 'string' && item.url);
...
async function generateWithVolcengine({
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
    ...
    let resultData;
    try {
      resultData = await safeJson(resultRes);
    } catch (parseErr) {
      // 轮询中一次非 JSON 响应按"本次失败"处理，继续重试而不是终止任务
      if (DEBUG) console.warn(`Volcengine 轮询第 ${attempt + 1} 次响应解析失败: ${parseErr.message}`);
      continue;
    }
    if (!resultRes.ok || resultData?.code !== 10000) {
      throw new Error(resultData?.message || resultData?.ResponseMetadata?.Error?.Message || `Volcengine get result failed (${resultRes.status})`);
...
function buildBaseUrl(req) {
  ...
  // 仅在 TRUST_PROXY 开启时信任 X-Forwarded-Proto/Host、Forwarded 头，
  // 否则这些头可被客户端伪造以生成错误的上传文件公网 URL
  const forwardedProto = TRUST_PROXY && typeof req.headers['x-forwarded-proto'] === 'string' ? req.headers['x-forwarded-proto'].split(',')[0].trim() : '';
  const protocol = forwardedProto || req.protocol || 'http';
  const forwardedHostHeader = TRUST_PROXY && typeof req.headers['x-forwarded-host'] === 'string' ? req.headers['x-forwarded-host'].split(',')[0].trim() : '';
  const forwardedHeader = TRUST_PROXY && typeof req.headers.forwarded === 'string' ? req.headers.forwarded : '';
...
const MIME_EXTENSION_MAP = configModule.MIME_EXTENSION_MAP;
...
function getClientIp(req) {
  // 仅在显式配置 TRUST_PROXY（反向代理后部署）时才信任 X-Forwarded-For，
  // 直连部署下伪造该头可绕过限流与防爆破锁定
  if (TRUST_PROXY) {
