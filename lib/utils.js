...
async function submitGrokVideo({ apiKey, model, prompt, imageUrls, aspectRatio, duration, resolution }) {
  ...
  const data = await res.json();
  if (!res.ok) throw new Error(data?.error || data?.message || `xAI video submit failed (${res.status})`);
  return data;
}
...
async function fetchGrokVideoResult({ apiKey, taskId }) {
  ...
  const data = await res.json();
  if (!res.ok) throw new Error(data?.error || data?.message || `xAI video query failed (${res.status})`);
  return data;
}
