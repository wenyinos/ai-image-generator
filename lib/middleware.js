...
const upload = multer({
  storage,
  limits: { fileSize: UPLOAD_FILE_MAX_SIZE, files: 10 },
...
const uploadMotion = multer({
  storage,
  limits: { fileSize: MOTION_FILE_MAX_SIZE, files: 10 },
  fileFilter: (req, file, cb) => {
...
function parseCookies(req) {
  ...
  return cookieHeader.split(';').reduce((acc, item) => {
    ...
    if (!key) return acc;
    // 含裸 % 等非法转义的 Cookie 值会让 decodeURIComponent 抛 URIError，视为无该 Cookie
    try {
