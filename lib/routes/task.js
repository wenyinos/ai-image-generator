...
router.post('/agnes-task-status', async (req, res) => {
  ...
  // 手动仅输入 taskId 查询时，从本地任务记录回填模型名（2.5 系列查询 URL 必须带 model_name）
  let modelName = model || '';
  if (!modelName && taskId) {
    const record = videoTaskDb.prepare('SELECT model FROM video_tasks WHERE task_id = ?').get(taskId);
    modelName = record?.model || '';
  }

  try {
    const result = await fetchAgnesVideoResult({ apiKey: resolvedApiKey, taskId, videoId, modelName });
    const status = result.status === 'completed' ? 'SUCCEEDED'
      : result.status === 'failed' ? 'FAILED'
      : result.status === 'in_progress' ? 'RUNNING'
      : 'PENDING';
    const payload = {
      provider: 'agnes', taskId, model: modelName, taskStatus: status,
