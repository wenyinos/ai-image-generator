...
router.post('/agnes-task-status', async (req, res) => {
  ...
  // 手动仅输入 taskId 查询时，从本地任务记录回填 video_id 与模型名（规范要求 /agnesapi?video_id=&model_name=）
  let modelName = model || '';
  let resolvedVideoId = videoId || '';
  if (taskId && (!modelName || !resolvedVideoId)) {
    const record = videoTaskDb.prepare('SELECT model, video_id FROM video_tasks WHERE task_id = ?').get(taskId);
    if (!modelName) modelName = record?.model || '';
    if (!resolvedVideoId) resolvedVideoId = record?.video_id || '';
  }

  try {
    const result = await fetchAgnesVideoResult({ apiKey: resolvedApiKey, taskId, videoId: resolvedVideoId, modelName });
    const status = result.status === 'completed' ? 'SUCCEEDED'
      : result.status === 'failed' ? 'FAILED'
      : result.status === 'in_progress' ? 'RUNNING'
      : 'PENDING';
    const payload = {
      provider: 'agnes', taskId, videoId: resolvedVideoId, model: modelName, taskStatus: status,
    ...
    saveVideoTaskRecord({
      taskId, provider: 'agnes', model: modelName, status,
      videoUrl: payload.videoUrl || '', message: payload.message,
    });
    res.json(payload);
  } catch (err) {
