...
router.post('/generate-image', async (req, res) => {
  ...
  if (selectedProvider === 'dashscope') {
    const validateModel = isSnapshotModel(selectedModel) ? selectedModel.replace(/-\d{4}-\d{2}-\d{2}$/, '') : selectedModel;
    if (!ALLOWED_MODELS.includes(validateModel)) {
  ...
  try {
    ...
    if (selectedProvider === 'openai') {
      const imageUrls = await generateWithOpenAI({
        apiKey: resolvedApiKey, model: selectedModel, prompt, n, size,
      });
      saveImageTaskRecord({ mode: 'text2image', provider: 'openai', model: selectedModel, status: 'SUCCEEDED', prompt, imageUrls });
      return res.json({ imageUrls });
    }
    ...
    if (isSyncModel(selectedModel)) {
      ...
      // 同步模型也支持异步任务模式（千问 3.0 系列 / 万相 2.7 系列）：
      // 请求体与同步接口一致，仅增加 X-DashScope-Async: enable 头并提交到异步端点，
      // 返回 task_id 后由前端轮询，避免长时间裸等待。
      const T2I_ASYNC_CAPABLE_MODELS = ['qwen-image-3.0', 'qwen-image-3.0-pro', 'wan2.7-image', 'wan2.7-image-pro'];
      const t2iBaseModel = isSnapshotModel(selectedModel) ? selectedModel.replace(/-d{4}-d{2}-d{2}$/, '') : selectedModel;
      if (selectedProvider === 'dashscope' && wantsProgress && T2I_ASYNC_CAPABLE_MODELS.includes(t2iBaseModel)) {
        const asyncSubmitRes = await fetchWithTimeout(
          `${BASE_URL}${IMAGE_GENERATION_ASYNC_ENDPOINT}`,
          {
            method: 'POST',
        ...
        const asyncSubmitData = await safeJson(asyncSubmitRes, 'DashScope 文生图异步提交返回了非 JSON 响应');
        if (!asyncSubmitRes.ok) return res.status(asyncSubmitRes.status).json({ error: formatUpstreamError(asyncSubmitData) });
        const taskId = asyncSubmitData.output?.task_id;
        if (!taskId) return res.status(500).json({ error: '未返回 task_id' });
        const taskStatus = asyncSubmitData.output?.task_status || 'PENDING';
        saveImageTaskRecord({
          id: taskId, taskId, mode: 'text2image', provider: 'dashscope',
          model: selectedModel, status: taskStatus, prompt,
        });
        if (DEBUG) console.log('✅ 文生图异步任务已提交:', taskId, selectedModel);
        return res.json({
          provider: 'dashscope', resultType: 'image',
          taskId, taskStatus, model: selectedModel,
        });
      }
      ...
      const syncData = await safeJson(syncRes, 'DashScope 同步生成返回了非 JSON 响应');
    ...
    const submitData = await safeJson(submitRes, 'DashScope 异步提交返回了非 JSON 响应');
    ...
    while (attempts < DASHSCOPE_MAX_POLL_ATTEMPTS) {
      ...
      let pollData;
      try {
        pollData = await safeJson(pollRes, 'DashScope 任务查询返回了非 JSON 响应');
      } catch (parseErr) {
        attempts++;
...
router.post('/image-to-image', (req, res, next) => {
  ...
  const I2I_SUPPORTED_MODELS = ['wan2.7-image-pro', 'wan2.7-image', 'wan2.6-image', 'qwen-image-2.0-pro', 'qwen-image-3.0', 'qwen-image-3.0-pro'];
  const i2iValidateModel = isSnapshotModel(selectedModel) ? selectedModel.replace(/-\d{4}-\d{2}-\d{2}$/, '') : selectedModel;
  if (selectedProvider === 'dashscope' && !I2I_SUPPORTED_MODELS.includes(i2iValidateModel)) {
  ...
  try {
    ...
    if (selectedProvider === 'openai') {
      const openaiResult = await generateWithOpenAI({
        apiKey: resolvedApiKey, model: selectedModel, prompt, n, size, imageDataUrl,
      });
      saveImageTaskRecord({ mode: 'image2image', provider: 'openai', model: selectedModel, status: 'SUCCEEDED', prompt, imageUrls: openaiResult });
      return res.json({ imageUrls: openaiResult });
    }
    ...
    // 同步模型图生图：image 字段需 HTTP URL（不支持 data: URL）
    let syncImageUrl = imageDataUrl;
    if (imageFile) {
      const meta = await saveUploadedImageAsPublicUrl(req, imageFile);
      scheduleUploadedFileCleanup(meta.filePath, GENERATION_REQUEST_TIMEOUT_MS + UPLOAD_FILE_CLEANUP_DELAY_MS);
      syncImageUrl = meta.publicUrl;
    }

    const content = [{ image: syncImageUrl }];
    ...
    // 百炼图像同步模型（千问 3.0 系列 / 万相 2.7 系列）也支持异步任务模式：
    // 请求体与同步接口一致，仅增加 X-DashScope-Async: enable 头并提交到异步端点，
    // 返回 task_id 后由前端轮询 /api/dashscope-task-status，避免长时间裸等待。
    const I2I_ASYNC_CAPABLE_MODELS = ['qwen-image-3.0', 'qwen-image-3.0-pro', 'wan2.7-image', 'wan2.7-image-pro'];
    if (selectedProvider === 'dashscope' && progressMode && I2I_ASYNC_CAPABLE_MODELS.includes(i2iValidateModel)) {
      const asyncSubmitRes = await fetchWithTimeout(
        `${BASE_URL}${IMAGE_GENERATION_ASYNC_ENDPOINT}`,
        {
          method: 'POST',
      ...
      const asyncSubmitData = await safeJson(asyncSubmitRes, 'DashScope 图生图异步提交返回了非 JSON 响应');
      if (!asyncSubmitRes.ok) {
        return res.status(asyncSubmitRes.status).json({ error: formatUpstreamError(asyncSubmitData) });
      }
      const taskId = asyncSubmitData.output?.task_id;
      if (!taskId) return res.status(500).json({ error: '未返回 task_id' });
      const taskStatus = asyncSubmitData.output?.task_status || 'PENDING';
      saveImageTaskRecord({
        id: taskId, taskId, mode: 'image2image', provider: 'dashscope',
        model: selectedModel, status: taskStatus, prompt,
      });
      if (DEBUG) console.log('✅ 图生图异步任务已提交:', taskId, selectedModel);
      return res.json({
        provider: 'dashscope', resultType: 'image',
        taskId, taskStatus, model: selectedModel,
      });
    ...
    const syncData = await safeJson(syncRes, 'DashScope 图生图返回了非 JSON 响应');
