...
const { parseVolcengineCredentials, submitVolcengineTask, callVolcengineSync, saveUploadedImageAsPublicUrl, scheduleUploadedFileCleanup, extractVolcengineImageUrls, parseImageUrlsInput, volcengineSign, volcengineCanonicalQuery, fetchWithTimeout } = require('../utils');
const { VOLCENGINE_HOST, VOLCENGINE_SERVICE, VOLCENGINE_REGION, VOLCENGINE_VERSION, GENERATION_REQUEST_TIMEOUT_MS, UPLOAD_FILE_CLEANUP_DELAY_MS } = require('../config');
...
// 区分客户端错误（400）与上游/内部错误（500）
function sendRouteError(res, err) {
  const message = err?.message || '请求失败';
  const isClientError = typeof message === 'string' && (message.includes('凭证') || message.includes('AK/SK') || message.includes('请提供') || message.includes('请上传'));
...
router.post('/volcengine-dressing', upload.array('images', 2), handleMulterError, async (req, res) => {
  try {
    ...
    scheduleUploadedFileCleanup(modelMeta.filePath);
    scheduleUploadedFileCleanup(garmentMeta.filePath);
...
router.post('/volcengine-seededit', upload.single('image'), handleMulterError, async (req, res) => {
  try {
    ...
    if (req.file) {
      ...
      scheduleUploadedFileCleanup(meta.filePath);
...
router.post('/volcengine-faceswap', upload.array('images', 4), handleMulterError, async (req, res) => {
  try {
    ...
    if (req.files && req.files.length > 0) {
      ...
      // 与 image.js 保持一致：强制 http(s) 前缀，防止透传任意协议
      urls = parseImageUrlsInput(imageUrls);
    }
...
router.post('/volcengine-facepretty', upload.single('image'), handleMulterError, async (req, res) => {
  try {
    ...
    formData.append('req_key', 'face_pretty');
    formData.append('image_url', finalImageUrl);
    formData.append('beauty_level', String(parseFloat(beautyLevel) || 1.0));
    if (multiFace === '1' || multiFace === true) formData.append('multi_face', '1');
    ...
    const query = { Action: 'FacePretty', Version: '2020-08-26' };
    ...
    const response = await fetchWithTimeout(
      ...
      { method: 'POST', headers: { ...auth.headers, 'Content-Type': 'application/x-www-form-urlencoded' }, body: bodyText },
      volcengineTimeoutMs
    );
    ...
    if (data?.code !== 10000) {
      throw new Error(data?.message || data?.ResponseMetadata?.Error?.Message || `Volcengine API error (${response.status})`);
    }
