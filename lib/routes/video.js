...
router.get('/video-models', async (req, res) => {
  const models = await getDashscopeVideoModels(req.query?.apiKey);
  res.json(models);
});
...
router.post('/generate-video', (req, res, next) => {
  ...
  const maxDuration = selectedModel.startsWith('wan3.0-video') ? 30 : 15;
  const durationErr = selectedMode !== 'videoedit' ? validateIntegerInRange('duration', duration, minDuration, maxDuration) : null;
  ...
  } else if (selectedMode === 'image2video' && (selectedModel === 'happyhorse-1.0-i2v' || selectedModel.startsWith('happyhorse-1.1-i2v'))) {
  ...
  } else if (selectedMode === 'image2video' && (selectedModel.startsWith('wan2.7-i2v') || selectedModel.startsWith('wan3.0-video'))) {
    // 与 happyhorse 分支一致：上游不接受 data: URL，首/尾帧落盘为公网 URL
    try {
      const wanFrameMetas = [];
      for (const frame of [firstFrame, lastFrame]) {
        if (!frame) continue;
        const meta = await saveUploadedImageAsPublicUrl(req, frame);
        if (meta.filePath) {
          scheduleUploadedFileCleanup(meta.filePath, VIDEO_GENERATION_REQUEST_TIMEOUT_MS + UPLOAD_FILE_CLEANUP_DELAY_MS);
        }
        wanFrameMetas.push(meta.publicUrl);
      }
      if (wanFrameMetas.length > 0) {
        input.media = [{ type: 'first_frame', url: wanFrameMetas[0] }];
        if (wanFrameMetas.length > 1) input.media.push({ type: 'last_frame', url: wanFrameMetas[1] });
      }
    } catch (err) {
      return res.status(400).json({ error: err.message });
    }
  ...
  if (selectedMode === 'text2video' && params.ratio) {
    if (selectedModel.startsWith('wan2.7-t2v') || selectedModel === 'happyhorse-1.0-t2v' || selectedModel.startsWith('wan3.0-video')) {
  ...
  // 运镜模板参数
  if (params.template_id) requestParams.template_id = params.template_id;
  if (params.camera_strength) requestParams.camera_strength = params.camera_strength;
  ...
  try {
    ...
    const submitData = await safeJson(submitRes, 'DashScope 视频提交返回了非 JSON 响应');
    ...
    for (let attempt = 0; attempt < DASHSCOPE_VIDEO_MAX_POLL_ATTEMPTS; attempt += 1) {
      ...
      let pollData;
      try {
        pollData = await safeJson(pollRes, 'DashScope 视频轮询返回了非 JSON 响应');
      } catch (parseErr) {
        continue; // 轮询中一次非 JSON 响应按"本次失败"处理，继续重试
      }
...
router.post('/jimeng-video', (req, res, next) => {
  ...
  // 仅三个 1080P/Pro reqKey 支持 binary_data_base64；720P i2v 模型带帧会被静默丢弃，
  // 按纯文生视频提交产生费用且结果不符预期，因此显式拒绝
  const supportsBinaryFrames = reqKey === 'jimeng_i2v_first_v30_1080' || reqKey === 'jimeng_i2v_first_tail_v30_1080' || reqKey === 'jimeng_ti2v_v30_pro';
  if (!supportsBinaryFrames && (firstFrame || lastFrame)) {
    return res.status(400).json({ error: `模型 ${reqKey} 不支持上传帧图片（仅 1080P/Pro 版本支持），请改用对应 1080P 模型或去掉首尾帧` });
  }
  ...
  try {
    ...
    const submitData = await safeJson(submitRes, '上游返回了非 JSON 响应');
...
// Agnes 视频生成
router.post('/agnes-video', (req, res, next) => {
  uploadMotion.fields([
    { name: 'firstFrame', maxCount: 1 },
  ])(req, res, (err) => {
    if (err) return handleMulterError(err, req, res, next);
  ...
  try {
    const { apiKey, model, prompt, mode } = req.body;
    const progressMode = req.body.progressMode === true || req.body.progressMode === 'true';
    ...
    let params;
    try { params = req.body.parameters ? JSON.parse(req.body.parameters) : {}; } catch (e) { params = {}; }

    const agnesBaseUrl = process.env.AGNES_BASE_URL || 'https://apihub.agnes-ai.com';
    const timeoutMs = Number.parseInt(process.env.AGNES_TIMEOUT_MS || String(VIDEO_GENERATION_REQUEST_TIMEOUT_MS), 10);
    const isAgnes25 = selectedModel === 'agnes-video-2.5' || selectedModel === 'agnes-video-2.5-flash';

    let body;
    if (isAgnes25) {
      // 2.5 系列：mode + seconds(字符串 4-12) + size + aspect_ratio
      const seconds = Math.min(12, Math.max(4, Number.parseInt(params.duration, 10) || 5));
      body = {
        model: selectedModel,
        prompt: prompt.trim(),
        mode: 'text',
      ...
      if (selectedModel === 'agnes-video-2.5-flash') {
        body.size = '720P';
      } else {
        body.size = ['720P', '1080P', '1K', '2K'].includes(params.resolution) ? params.resolution : '720P';
      }
      body.aspect_ratio = ['21:9', '16:9', '4:3', '1:1', '3:4', '9:16'].includes(params.ratio) ? params.ratio : '16:9';
      ...
      body = {
        ...
        num_frames: params.frames || params.num_frames || 121,
    ...
    const firstFrame = req.files?.firstFrame?.[0];
    if (firstFrame) {
      const meta = await saveUploadedImageAsPublicUrl(req, firstFrame);
      scheduleUploadedFileCleanup(meta.filePath, timeoutMs + UPLOAD_FILE_CLEANUP_DELAY_MS);
      if (isAgnes25) {
        body.mode = 'reference';
        body.images = [meta.publicUrl];
      } else {
        body.image = meta.publicUrl;
      }
    }

    const submitRes = await fetchWithTimeout(
      `${agnesBaseUrl}/v1/videos`,
      {
        method: 'POST',
    ...
    const submitData = await safeJson(submitRes, '上游返回了非 JSON 响应');
    if (!submitRes.ok) return res.status(submitRes.status).json({ error: submitData?.error?.message || submitData?.message || 'Agnes 视频提交失败' });

    const taskId = submitData.task_id || submitData.id;
    if (!taskId) return res.status(500).json({ error: 'Agnes 未返回 task_id' });
    const videoId = submitData.video_id || '';

    saveVideoTaskRecord({ taskId, videoId, provider: 'agnes', model: selectedModel, mode: mode || 'text2video', status: 'PENDING', prompt: prompt.trim() });

    if (progressMode) {
      return res.json({ provider: 'agnes', resultType: 'video', taskId, videoId, taskStatus: 'PENDING', model: selectedModel });
    }

    // 后端阻塞轮询
    for (let attempt = 0; attempt < DASHSCOPE_VIDEO_MAX_POLL_ATTEMPTS; attempt += 1) {
      await new Promise(resolve => setTimeout(resolve, DASHSCOPE_VIDEO_POLL_INTERVAL_MS));
      const result = await fetchAgnesVideoResult({ apiKey: resolvedApiKey, taskId, videoId, modelName: selectedModel });
      if (result.status === 'completed') {
        const videoUrl = extractAgnesVideoUrl(result);
        if (!videoUrl) return res.status(500).json({ error: `Agnes 任务完成但未返回视频 URL（响应字段：${Object.keys(result || {}).join(', ') || '空'}）` });
        saveVideoTaskRecord({ taskId, provider: 'agnes', model: selectedModel, status: 'SUCCEEDED', videoUrl });
        return res.json({ videoUrl, taskId, usage: null });
      }
      if (result.status === 'failed') {
        saveVideoTaskRecord({ taskId, provider: 'agnes', model: selectedModel, status: 'FAILED', error: result.error || '任务失败' });
        return res.status(500).json({ error: result.error || 'Agnes 视频生成失败' });
      }
    }
    return res.status(504).json({ error: 'Agnes 视频生成超时' });
  } catch (err) {
...
// Grok 视频生成
router.post('/grok-video', (req, res, next) => {
  uploadMotion.fields([
    { name: 'grokImages', maxCount: 7 },
  ])(req, res, (err) => {
    if (err) return handleMulterError(err, req, res, next);
    next();
  });
  ...
  try {
    const { apiKey, model, prompt, mode } = req.body;
    const progressMode = req.body.progressMode === true || req.body.progressMode === 'true';
    const selectedModel = model || 'grok-video-1.0';
    ...
    // 上传参考图
    const grokImageFiles = req.files?.grokImages || [];
    const imageUrls = [];
    for (const file of grokImageFiles) {
      const meta = await saveUploadedImageAsPublicUrl(req, file);
      scheduleUploadedFileCleanup(meta.filePath, VIDEO_GENERATION_REQUEST_TIMEOUT_MS + UPLOAD_FILE_CLEANUP_DELAY_MS);
      imageUrls.push(meta.publicUrl);
    }

    // grok-video-1.5 必须且只能 1 张参考图
    if (selectedModel === 'grok-video-1.5' && imageUrls.length !== 1) {
      return res.status(400).json({ error: 'grok-video-1.5 必须且只能上传 1 张参考图' });
    }
    if (selectedModel === 'grok-video-1.0' && imageUrls.length > 7) {
      return res.status(400).json({ error: 'grok-video-1.0 最多上传 7 张参考图' });
    }

    const submitData = await submitGrokVideo({
    ...
    // 异步任务模式：返回 id，轮询查询
    const taskId = submitData?.id || submitData?.video_id || submitData?.task_id;
    if (taskId) {
      saveVideoTaskRecord({ taskId, provider: 'xai', model: selectedModel, mode: mode || 'text2video', status: 'PENDING', prompt: prompt.trim() });
      if (progressMode) {
        return res.json({ provider: 'xai', resultType: 'video', taskId, taskStatus: 'PENDING', model: selectedModel });
      }
    }

    // 同步返回（如果直接包含视频 URL）
    const videoUrl = submitData?.video_url || submitData?.data?.video_url || '';
    if (videoUrl) {
      saveVideoTaskRecord({ taskId: taskId || 'grok-' + Date.now(), provider: 'xai', model: selectedModel, status: 'SUCCEEDED', videoUrl });
      return res.json({ videoUrl, taskId: taskId || '', usage: null });
    }
    if (!taskId) return res.status(500).json({ error: 'xAI 未返回任务 ID' });
    ...
    for (let attempt = 0; attempt < DASHSCOPE_VIDEO_MAX_POLL_ATTEMPTS; attempt += 1) {
      ...
      const resultUrl = result?.video_url || result?.data?.video_url || result?.url || '';
      if (resultUrl && (status === 'completed' || status === 'succeeded' || status === 'done')) {
        saveVideoTaskRecord({ taskId, provider: 'xai', model: selectedModel, status: 'SUCCEEDED', videoUrl: resultUrl });
        return res.json({ videoUrl: resultUrl, taskId, usage: null });
      }
      if (status === 'failed' || status === 'cancelled' || status === 'canceled') {
        saveVideoTaskRecord({ taskId, provider: 'xai', model: selectedModel, status: 'FAILED', error: result?.error || '任务失败' });
        return res.status(500).json({ error: result?.error || 'Grok 视频生成失败' });
      }
    }
    return res.status(504).json({ error: 'Grok 视频生成超时' });
  } catch (err) {
    return res.status(500).json({ error: err.message });
  }
});
